...
- **`prompt_cache_key` is required, not advisory.** Current docs: on GPT-5.6 and later model families you *must* set it to get the more reliable matching, for both implicit and explicit caching. Without a key a request may still take an automatic hit, but it does not use the improved per-breakpoint matching. Treat a missing key on a 5.6 route as a P0 finding rather than a tuning opportunity. The approximate 15 requests-per-minute envelope per key in Mechanics applies, so scope keys to a prompt family or session, never to a single global constant.
