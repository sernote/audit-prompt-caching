...
- Latency: p50/p95/p99 TTFT or prefill, final/end-to-end latency, output/decode time, tool time.
...
- Labeled OpenAI-compatible vendors (`provider` set to `moonshot`, `minimax`, `xai`, `mistral`, `tencent`, `xiaomi`, `upstage`, `zai`, `yandex`, or their aliases) resolve to `inclusive` for OpenAI-shaped usage and to `additive` when the record carries Anthropic-shaped `cache_read_input_tokens`/`cache_creation_input_tokens`. The label is the opt-in; unlabeled wrapper usage stays `ambiguous`.
