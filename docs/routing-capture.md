...
Use this optional reference when a routing investigation needs to link router
decisions, worker measurements and client outcomes for an individual request.
The [experimental analyzer](../audit-prompt-caching/references/routing-evidence.md)
requires a normalized export; it does not parse ordinary router logs. A
[project audit](first-audit.md) can begin with code and configuration.
...
Run from this repository's root. Success here means valid analysis, not complete
coverage or acceptable latency. Review joined, decision-only and outcome-only
records and their unknown fields. If the mapping is unavailable, record which
question remains unanswered and what instrumentation would be needed. Creating
that export is separate engineering work.
